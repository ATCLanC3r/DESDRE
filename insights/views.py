...
def recommendation_insights(request):
    ...
    Accepts RegularUser ID (any customer type: customer, restaurant, community_member).
    """
    ...
    if request.method == "POST":
        ...
        order_items = OrderItem.objects.filter(
            producer_order__payment__user_id=user.id,
        ...
        try:
            response = http_client.post(
                ...
                json={"user_id": user.id, "purchase_history": purchase_history, "top_k": TOP_K},
                timeout=15,
            )
