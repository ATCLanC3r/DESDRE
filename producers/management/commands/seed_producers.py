...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(10):  # Create 10 producers
            ...
            # Use a deterministic unique business name. Random selection created
            # multiple accounts for the same store and made order ownership
            # ambiguous in demonstrations.
            profile.business_name = farm_names[i]
            profile.lead_time_hours = randint(48,168)
            profile.save()
            ...
            created_count += 1
            self.stdout.write(self.style.SUCCESS(f"  Created producer: {user.username} - {profile.business_name} at {addr_data['postcode']} with lead time of {profile.lead_time_hours}"))
