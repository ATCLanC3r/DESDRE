...
from mainApp.models import CustomerProfile, Address, RegularUser
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        random.seed(seed)
        np.random.seed(seed)
        customers = list(RegularUser.objects.filter(
                ...
                for product in set(selected_products):  # Use set to avoid duplicates in same order
                    ...
                    if producer.id not in producer_map:
                        producer_map[producer.id] = OrderProducer.objects.create(
                            ...
                            delivered_by=created_at+ timedelta(hours=producer.lead_time_hours),
                            created_at=created_at,
