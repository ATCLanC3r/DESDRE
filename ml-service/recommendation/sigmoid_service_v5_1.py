    ...
    def get_product_saliency(self, user_id, target_product_id, purchase_history):
        ...
        try:
            with tf.GradientTape() as tape:
                logits, _ = self._model([prod_input, time_input, user_cluster], training=True)
                target_score = logits[0, target_idx]
            
            grads = tape.gradient(target_score, prod_input)
            if grads is None:
                grads = tf.ones_like(prod_input, dtype=tf.float32)
            grads = tf.abs(grads)
