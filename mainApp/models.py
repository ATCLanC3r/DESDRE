...
class Address(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        for addr in Address.objects.filter(
            ...
            is_default=True
        ).exclude(pk=self.pk if not is_new else None):
        ...
        if not skip_geocoding and self.post_code:
            ...
            if needs_geocoding:
                transaction.on_commit(
                    lambda: geocode_address_async.delay(self.pk)
                )
...
class RegularUser(AbstractUser):
    ...
    # post_code = models.CharField(max_length=20) # use the properties to get postcode of default adress
    ...
    def default_address_postcode(self):
        ...
        return address.post_code if address else None
    ...
    def soft_delete(self):
        ...
        Used when user deletes their account (avoid integrity issues)
        """
...
class LoginAttempt(models.Model):
    """TC-022: Audit log for login successes and failures (rate limiting evidence)."""
    ...
    def __str__(self):
        ...
        return f"[{status}] {self.username} from {self.ip_address} at {self.attempted_at:%Y-%m-%d %H:%M}"
...
class Notification(models.Model):
    ...
    user = models.ForeignKey(RegularUser, on_delete=models.CASCADE, related_name='notifications')
    notification_type = models.CharField(max_length=20, choices=NOTIFICATION_TYPES, default=TYPE_INFO)
    title = models.CharField(max_length=255)
    message = models.TextField()
    link = models.CharField(max_length=500, blank=True, help_text="Optional URL to redirect user on click")
